...
def define_map(df, sess_path, title, nb_images, altitude, survey_area, start, end):
    """
    ...
    body_html = '''
                ...
                '''.format(title=title, dir=sess_path.name, altitude=altitude, platform = df.iloc[0]["Make"] + \
                           " " + df.iloc[0]["Model"], survey_area=survey_area, nb_images = nb_images, start=start,\
                           duration=converthms(((datetime.datetime.strptime(end,'%Y:%m:%d %H:%M:%S'))-(datetime.datetime.strptime(start,'%Y:%m:%d %H:%M:%S'))).seconds) , end=end)
...
def convert_map_pdf(file_name):
    ...
    pdf = pdfkit.from_file(str(mapName),  Path(mapName.parent, mapName.stem + '.pdf'), options=options)
